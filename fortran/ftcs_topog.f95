subroutine ftcs_topog(m0, n0, phi0, m, n, phi, H, f, g, dx, dt, K, &
  length)
  ! The two-dimensional shallow water equations with bottom topography
  ! and rotation as derived in [2] are implemented with the additional
  ! assumption that the functions u, v, and phi are independent of
  ! y. These equations appear in [2], and are solved with a forward
  ! time and centered space finite difference scheme. Diffusion terms
  ! are included.
  !
  ! References:
  ...
  double precision, dimension(length), intent(in) :: m0   ! Initial
  double precision, dimension(length), intent(in) :: n0   ! Initial
  double precision, dimension(length), intent(in) :: phi0 ! Initial
  double precision, dimension(length), intent(out) :: m   ! Updated
  double precision, dimension(length), intent(out) :: n   ! Updated
  double precision, dimension(length), intent(out) :: phi ! Updated
  double precision, dimension(length), intent(in) :: H    ! Topography
  ...
  m(1) = m0(1) - coeff1 * ((u0(2) + u0(1)) * (m0(2) + m0(1)) - &
       ...
       + (phi0(1) + phi0(length)) * (H(1) - H(length)))) + dt * f * &
       n0(1) + coeff3 * (m0(length) - 2.d0 * m0(1) + m0(2))
  ...
  do j = 2, length - 1
     m(j) = m0(j) - coeff1 * ((u0(j + 1) + u0(j)) * (m0(j + 1) + &
          ...
          phi0(j + 1) ** 2 - phi0(j - 1) ** 2 + g * ((phi0(j + 1) + &
          phi0(j)) * (H(j + 1) - H(j)) + (phi0(j) + phi0(j - 1)) * &
          (H(j) - H(j - 1)))) + dt * f * n0(j) + coeff3 * (m0(j - 1) &
          - 2.d0 * m0(j) + m0(j + 1))

     n(j) = n0(j) - coeff1 * ((v0(j + 1) + v0(j)) * (m0(j + 1) + &
          m0(j)) - (v0(j) + v0(j - 1)) * (m0(j) + m0(j - 1))) - dt * &
          f * m0(j) + coeff3 * (n0(j - 1) - 2.d0 * n0(j) + n0(j + 1))

     phi(j) = phi0(j) - coeff2 * (m0(j + 1) - m0(j - 1)) + coeff3 * &
            (phi0(j - 1) - 2.d0 * phi0(j) + phi0(j + 1))
  end do

  m(length) = m0(length) - coeff1 * ((u0(1) + u0(length)) * (m0(1) + &
       m0(length)) - (u0(length) + u0(length - 1)) * (m0(length) + &
       m0(length - 1)) + phi0(1) ** 2 - phi0(length - 1) ** 2 + g * &
       ((phi0(1) + phi0(length)) * (H(1) - H(length)) + &
       (phi0(length) + phi0(length - 1)) * (H(length) - &
       H(length - 1)))) + dt * f * n0(length) + coeff3 * &
